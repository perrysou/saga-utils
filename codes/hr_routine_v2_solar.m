...
cases_folder = [p1, sep, 'SolarEclipse', sep, 'tests', sep];
...
for yearnum = yearlist
    ...
    for doynum = unique(doylist)
        ...
        if isempty(dir(lrtimesfile))
            ...
            [~, TSP_hr0, TSP_hrv0] = find_general_times(MSP, rcvr_op, min(spth_hr));
            [~, TS4_hr0, TS4_hrv0] = find_general_times(MS4, rcvr_op, min(s4th_hr));
            save(lrtimesfile, 'MSP', 'MS4', 'rcvr_op', ...
                'spth_hr', 's4th_hr', ...
                'TSP_hr0', 'TSP_hrv0', ...
                'TS4_hr0', 'TS4_hrv0');
        else
...
%             prnlist = prnlist(1);
    ...
    if length(doyin) >= 365 && ~isempty(SD) && ~isempty(MEGA_MSP) && ~isempty(MSP_days)
        save([p2, 'sd_', year, '.mat'], 'SD', 'MEGA_MSP', 'MSP_days', 'SCINTEVENTS');
    end
    
    csvwrite([p1, sep, 'ScintillationEvents_short_', year, '.csv'], SCINTEVENTS);
