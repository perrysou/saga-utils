function [] = hr_routine_v1(yearin,doyin)
%% Initialization
close all;
warning off;
dbstop if error;
...
if isempty(dir(lrtimesfile))
    ...
    TSP_hrv0
    TSP_hr = TSP_hr0;
%     save(lrtimesfile,'MSP','rcvr_op','spth_hr','TSP_hr','TSP_hrv0');
else
    load(lrtimesfile);
    findhrtimes(year,doy);
    TSP_hrv0
end
...
%         disp('All 5 receivers including ASTRA');
        zoomin_hrplot_iq_carrier(irow,tt,dt,signal_type,home_dir,cases_folder,year,doy,prn,rcvr_op_hr);
        %
     end
end
...
save(['/data1/home/ysu27/Dropbox/research/sd_',year,'.mat'],'SD','MEGA_MSP','MSP_days');    
end
% fclose(fid1);
