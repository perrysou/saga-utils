...
function DATAM2 = Fn_ReadHighRate_CASESdata(KK,op_path,cases_folder,rcvr_name,signal_type,tstart,tend)
tic
...
% system(cell2mat(command));
command = strcat('mkdir',{' '},op_path,'hr_prn_files_',signal);
system(cell2mat(command));
...
% iqfilestruct = dir([folder_name 'txt/iq*.log']);
% % iqfilename = [folder_name '\txt\' iqfilestruct(1).name];
% scintfilestruct = dir([folder_name 'txt/scint*.log']);
% scintfilename = [folder_name '\txt\' scintfilestruct(1).name];

%Extract data from local storage
iqfile = read_logfile('iq*',tstart,tend,rcvr_name,cases_folder,op_path);
% iqfile = read_from_HF_file(tstart,tend,filepath,'iq');
%take scintdata for the whole day
scfilename = strcat('prn_files_',signal,sep,'scintdata.mat');
sc = load([op_path,scfilename]);
scfile = sc.scfile;
...
scdata = scdata((scdata(:,1)>1000 & scdata(:,12)>-80 & scdata(:,5)>-1 ...
    ...
    & scdata(:,10)>-1) & scdata(:,8)<10 & scdata(:,9)<10 & scdata(:,10)<10,:);
...
%    reftempLR = find(scdata(:,13)==1|scdata(:,13)==2);
   reftempLR = find(scdata(:,13)==1|scdata(:,13)==2);
   if isempty(reftempLR)
       disp('No reference channels?!');
       reftempLR = find(scdata(:,13)==0&scdata(:,15)~=kk);
   end
   size(find(scdata(:,13)==1))
