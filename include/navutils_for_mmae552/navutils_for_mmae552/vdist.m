...
% Original author: Michael Kleder.
% 8 May 2008 Seebany Datta-Barua: modifying to accept vector arguments.
% 19 Mar 2009 SDB: modifying to return dist = 0 if
% (lat1,lon1)==(lat2,lon2).

% Input check:
if ~isempty(find(abs(lat1)>90)) | ~isempty(find(abs(lat2)>90))
...
rows = find(abs(pi/2-abs(lat1)) < 1e-10 );
lat1(rows) = sign(lat1(rows))*(pi/2-(1e-10));
clear rows

rows = find( abs(pi/2-abs(lat2)) < 1e-10 );
lat2(rows) = sign(lat2(rows))*(pi/2-(1e-10));
clear rows
...
lambdaold = 0;
itercount = 0;
while ~itercount | ~isempty(find(abs(lambda-lambdaold) > 1e-4))%1e-12))  % force at least one execution
...
%         warning('Points are essentially antipodal. Precision may be reduced slightly.');
    ...
    if ~isempty(find(imag(lambda)))
%         warning('Points are essentially antipodal. Precision may be reduced slightly.');
        lambda(find(imag(lambda))) = real(pi);
        lambdaold(find(imag(lambda))) = real(pi);
%         break
    end
    rows = find( lambda > pi );
    if ~isempty(rows)
        % This warning shows up every time.
%         warning('Points are essentially antipodal. Precision may be reduced slightly.');
        lambda(rows) = pi;
        lambdaold(rows) = pi;
%         break
