...
matlab_version = str2num(matlab_version(1));matlab_version=5.0;
...
version_number = str2num(matlab_version(1));version_number = 5.0;
...
set(cax,'Color','w','XColor','w','YColor','w'); % 'k'
...
%     qte = text(xx(last),yy(last),'x','Color','w'); % 'k' % end point symbol is an x
  end % if any(I_last)
end % if any(I_out_plot)
...
% make a radial grid
  ...
  hhh=plot([0 max_angle],[0 max_radius],'k'); % 'w'
...
% now really force points on x/y axes to lie on them exactly
  ...
  for i=(rmin+rinc):rinc:rmax      % control of elevation
    ...
    text(i+rinc/20,0,['  ' num2str(abs(i - 90))],  ...
         'verticalalignment','bottom','Color','k' ); % 'w'
